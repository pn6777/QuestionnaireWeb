...
export default{
    ...
    methods:{
        ...
        editEnd(isPublished){
            ...
            // 將資料送入 DB
            this.saveSurveyToDb();
            this.$emit('editEnd', 2)  // 改變畫面
        },
        saveSurveyToDb(){
            let obj = this.surveyCreateObj;
            console.log(obj);
            fetch("http://localhost:8080/survey/edit", {
            method: 'POST',
            headers: {
        ...
        packageSurvey(isPublished){
            // 塞基本資料
            this.surveyCreateObj.no = this.$route.params.surveyNo;
            ...
            console.log(this.surveyCreateObj);
        },
        /****** 使用者操作題目的相關方法 ***** */
        // 修改填寫對象
        changeEdit(n){
    ...
    created(){
        this.number = parseInt(this.survey.questionList.length);
        this.author = sessionStorage.getItem("ac");
        this.surveyTitle = this.survey.name;
        this.surveyStart = this.survey.startDate;
        this.surveyEnd = this.survey.endDate;
        this.surveyDescription = this.survey.description;
        for(let index in this.survey.questionList){
            this.qRequired[index] = this.survey.questionList[index].required;
            this.qType[index] =  this.survey.questionList[index].type;
            if(this.qType[index] === "text"){
                return;
            }
            this.qOption[index] = this.survey.questionList[index].option;
        }
    },
    mounted(){
        for(let index in this.survey.questionList){
            document.getElementById("no_"+(parseInt(index)+1)).childNodes[3].value = this.survey.questionList[index].question;
        }
...
<template>
    ...
    <div class="createBody">
        ...
        <div class="footerLink">
            ...
            <!-- 送出 -->
            <button class="sendBtn" type="button" v-if="this.isChecked" @click="this.editEnd(false)">僅儲存</button>
            <button class="sendBtn" type="button" v-if="this.isChecked" @click="this.editEnd(true)">發布</button>
        </div>
    </div>
</template>
